...
def generar_pdf_partido(pid):
    ...
    # ── DEUDAS ACUMULADAS DEL PLANTEL (café) ──────────────────────────────
    story.append(sp(8))
    story.append(sec_header("DEUDAS ACUMULADAS DEL PLANTEL", C_COBRO))
    story.append(sp(4))
    story.append(Paragraph(
        "Estado de cuentas al momento del partido. Incluye cuotas y multas pendientes de todos los partidos.",
        sN))
    story.append(sp(5))

    # Traer todos los jugadores activos con sus deudas
    deudas_plantel = q("""
        SELECT j.nombre, j.numero,
               COALESCE((SELECT SUM(monto-COALESCE(monto_pagado,0))
                         FROM pagos WHERE jugador_id=j.id AND pagado=0), 0) as debe_cuotas,
               COALESCE((SELECT SUM(monto-COALESCE(monto_pagado,0))
                         FROM multas WHERE jugador_id=j.id AND pagado=0), 0) as debe_multas
        FROM jugadores j
        WHERE j.activo=1
        ORDER BY (
            COALESCE((SELECT SUM(monto-COALESCE(monto_pagado,0)) FROM pagos WHERE jugador_id=j.id AND pagado=0),0) +
            COALESCE((SELECT SUM(monto-COALESCE(monto_pagado,0)) FROM multas WHERE jugador_id=j.id AND pagado=0),0)
        ) DESC, j.nombre
    """)

    d_data = [["#", "JUGADOR", "CUOTAS", "MULTAS", "TOTAL DEBE", "ESTADO"]]
    total_general = 0.0
    for _, row in deudas_plantel.iterrows():
        cuotas = float(row['debe_cuotas'] or 0)
        multas = float(row['debe_multas'] or 0)
        total  = cuotas + multas
        total_general += total
        num = str(int(row['numero'])) if row['numero'] and str(row['numero']) not in ('nan','None') else '—'
        estado = "✓ Al día" if total < 0.001 else f"${total:,.2f} pendiente"
        d_data.append([
            num,
            str(row['nombre']),
            f"${cuotas:,.2f}" if cuotas > 0.001 else "—",
            f"${multas:,.2f}" if multas > 0.001 else "—",
            f"${total:,.2f}" if total > 0.001 else "—",
            estado,
        ])
    d_data.append(["", "TOTAL PLANTEL", "", "", f"${total_general:,.2f}", ""])

    t_deudas = tabla_con_total(d_data,
        [1.2*cm, 4.5*cm, 2.3*cm, 2.3*cm, 2.5*cm, 4.2*cm],
        align_cols={2:'RIGHT', 3:'RIGHT', 4:'RIGHT'},
        header_color=C_COBRO,
        total_color=colors.HexColor("#fde8d8"))

    # Colorear filas al día en verde
    for i, row in enumerate(d_data[1:-1], 1):
        if row[5] == "✓ Al día":
            t_deudas.setStyle(TableStyle([
                ('BACKGROUND', (0,i), (-1,i), colors.HexColor("#e8f5ec")),
                ('TEXTCOLOR', (5,i), (5,i), colors.HexColor("#007a30")),
                ('FONTNAME', (5,i), (5,i), 'Helvetica-Bold'),
            ]))
        else:
            t_deudas.setStyle(TableStyle([
                ('TEXTCOLOR', (4,i), (4,i), colors.HexColor("#cc0000")),
...
/* Ocultar el panel de tabs que no está seleccionado */
